...
function with_kw(typedef, mod::Module, withshow=true)
    ...
    for (i,l) in enumerate(lns) # loop over body of typedef
        ...
        if l.head==:macrocall && l.args[1] != Symbol("@assert") && l.args[1] != Symbol("@smart_assert") && l.args[1] != Symbol("@argcheck")
    ...
    for (i,l) in enumerate(lns) # loop over body of typedef
        ...
        elseif l.head==:macrocall && (l.args[1]==Symbol("@assert") || l.args[1]==Symbol("@smart_assert") || l.args[1]==Symbol("@argcheck"))
            # store all asserts
            push!(asserts, l)
